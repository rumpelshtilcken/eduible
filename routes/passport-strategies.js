...
exports.configure = (
  {
    ...
    path = '/auth' // URL base path for authentication route"
  } = {}
) => {
  if (app === null) {
      ...
      new Strategy(
        ...
        (req, accessToken, refreshToken, profile, next) => {
          try {
            ...
            User.one({ [providerName]: profile.id }, (err, user) => {
              if (err) {
                return next(err);
              }
              ...
              if (req.user) {
                ...
                if (!user) {
                  return User.get(req.user.id, (err, user) => {
                    if (err) {
                      return next(err);
                    }
                    if (!user) {
                      return next(
                        new Error('Could not find current user in database.')
                      );
                    }
                    ...
                    user.save((err) => {
                      ...
                      if (err) {
                        return next(null, false, {
                          message: "Please check there isn't an account associated with the same email address."
                        });
                      }
                ...
                return User.one({ email: profile.email }, (err, user) => {
                  ...
                  if (user) {
                    return next(null, false, {
                      message: 'There is already an account associated with the same email address.'
                    });
                  }
  ...
  providers.forEach(({ providerName, providerOptions }) => {
    ...
    server.get(
      ...
      passport.authenticate(providerName, providerOptions)
    );
